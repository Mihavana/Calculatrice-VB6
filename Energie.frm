...
Begin VB.Form Energie 
   ...
   Begin VB.ComboBox listVol 
      ...
      Height          =   315
      Index           =   1
      ItemData        =   "Energie.frx":3F07
      Left            =   120
      List            =   "Energie.frx":3F17
      TabIndex        =   14
      Top             =   6000
      Width           =   3495
   End
   Begin VB.ComboBox listVol 
      ...
      Height          =   315
      Index           =   0
      ItemData        =   "Energie.frx":3F5C
      Left            =   120
      List            =   "Energie.frx":3F6C
      TabIndex        =   13
      Top             =   3120
      Width           =   3495
   End
...
l = 0
End If
If listVol(j) = "Electron-volt(s)" Then
   If listVol(l) = "Electron-volt(s" Then
    panConversion(l) = panConversion(j)
   ElseIf listVol(l) = "Joules" Then
    panConversion(l) = panConversion(j) * 1.602177E-19
   ElseIf listVol(l) = "Kilojoule(s)" Then
    panConversion(l) = panConversion(j) * 1.602177E-22
   ElseIf listVol(l) = "Calorie(s) thermique(s)" Then
    panConversion(l) = panConversion(j) * 3.829294E-20
   End If
ElseIf listVol(j) = "Joules" Then
   If listVol(l) = "Joules" Then
    panConversion(l) = panConversion(j)
   ElseIf listVol(l) = "Electron-volt(s)" Then
    panConversion(l) = panConversion(j) * 6.241509E+18
   ElseIf listVol(l) = "Kilojoule(s)" Then
    panConversion(l) = panConversion(j) * 0.001
   ElseIf listVol(l) = "Calorie(s) thermique(s)" Then
    panConversion(l) = panConversion(j) * 0.239006
   End If
ElseIf listVol(j) = "Kilojoule(s)" Then
   If listVol(l) = "Kilojoule(s)" Then
    panConversion(l) = panConversion(j)
   ElseIf listVol(l) = "Joules" Then
    panConversion(l) = panConversion(j) * 1000
   ElseIf listVol(l) = "Electron-volt(s)" Then
    panConversion(l) = panConversion(j) * 6.241509E+21
   ElseIf listVol(l) = "Calorie(s) thermique(s)" Then
    panConversion(l) = panConversion(j) * 239.0057
   End If
ElseIf listVol(j) = "Calorie(s) thermique(s))" Then
   If listVol(l) = "Calorie(s) thermique(s)" Then
    panConversion(l) = panConversion(j)
   ElseIf listVol(l) = "Kilojoule(s)" Then
    panConversion(l) = panConversion(j) * 0.004184
   ElseIf listVol(l) = "Joules" Then
    panConversion(l) = panConversion(j) * 4.184
   ElseIf listVol(l) = "Electron-volt(s)" Then
    panConversion(l) = panConversion(j) * 2.611448E+19
   End If
End If
End Function
