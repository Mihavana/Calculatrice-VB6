...
Begin VB.Form Puissance 
   ...
   Begin VB.ComboBox listVol 
      ...
      Height          =   315
      Index           =   0
      ItemData        =   "Puissance.frx":3F07
      Left            =   120
      List            =   "Puissance.frx":3F14
      TabIndex        =   1
      Top             =   3120
      Width           =   3135
   End
   Begin VB.ComboBox listVol 
      ...
      Height          =   315
      Index           =   1
      ItemData        =   "Puissance.frx":3F4F
      Left            =   120
      List            =   "Puissance.frx":3F5C
      TabIndex        =   0
      Top             =   6000
      Width           =   3135
   End
...
If panConversion(j) <> "0" Then panConversion(j).Caption = panConversion(j).Caption & i
If panConversion(j).Caption = 0 Or posNeg = True Then
    panConversion(j).Caption = i
    If posNeg = True Then panConversion(j).Caption = -i
    posNeg = False
End If
Call conversion
End Sub
...
l = 0
End If
If listVol(j) = "Watts" Then
   If listVol(l) = "Watts" Then
    panConversion(l) = panConversion(j)
   ElseIf listVol(l) = "Kilowatt(s)" Then
    panConversion(l) = panConversion(j) * 0.001
   ElseIf listVol(l) = "Cheval/chevaux-vapeur(Etats-Unis)" Then
    panConversion(l) = panConversion(j) * 0.001341
   End If
ElseIf listVol(j) = "Kilowatt(s)" Then
   If listVol(l) = "Kilowatt(s)" Then
    panConversion(l) = panConversion(j)
   ElseIf listVol(l) = "Watts" Then
    panConversion(l) = panConversion(j) * 1000
   ElseIf listVol(l) = "Cheval/chevaux-vapeur(Etats-Unis)" Then
    panConversion(l) = panConversion(j) * 1.341022
   End If
ElseIf listVol(j) = "Cheval/chevaux-vapeur(Etats-Unis)" Then
   If listVol(l) = "Cheval/chevaux-vapeur(Etats-Unis)" Then
    panConversion(l) = panConversion(j)
   ElseIf listVol(l) = "Watts" Then
    panConversion(l) = panConversion(j) * 745.6999
   ElseIf listVol(l) = "Kilowatt(s)" Then
    panConversion(l) = panConversion(j) * 0.7457
   End If
End If
End Function
...
panConversion(j) = "-" & panConversion(j)
posNeg = True
Call conversion
End Sub
